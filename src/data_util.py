...
def preprocess_wenergy(data, layer_boundaries, eps=1.0e-10):
    "Transform a là CERN VAE"
    energy, layers = get_energy_and_sorted_layers(data)
...
def preprocess(data, layer_boundaries, eps=1.e-10, u0up_cut=7.0, u0low_cut=0.0, rew=1.0):
...
def postprocess_wenergy(x, c, layer_boundaries, quantiles, threshold=1.0e-4):
    "Reverse the effect of preprocess_wenergy"
    assert len(x) == len(c)
    assert len(x.shape) == 2
...
def get_loaders(filename, xml_filename, particle_type, val_frac, batch_size, 
    ...
    x, c = preprocess(data, layer_boundaries, eps, u0up_cut=u0up_cut, u0low_cut=u0low_cut, rew=rew)
    #x, c = preprocess_wenergy(data, layer_boundaries, eps)

...
class ViT(nn.Module):
    ...
    def __init__(self, param):
        ...
        self.in_channels = C
        self.out_channels = C # * 2 if learn_sigma else C
        self.patch_shape = list(self.patch_shape)
        self.num_patches = [
            s // p for s, p in zip(self.shape[1:], self.patch_shape)
        ]
        
        # patcher from github.com/lucidrains/vit-pytorch/blob/main/vit_pytorch/vit_3d.py
        ...
        self.c_embedder = nn.Sequential(
            nn.Linear(self.condition_dim, self.hidden_dim, bias=True),
            nn.SiLU(),
        ...
        self.pos_embed = nn.Parameter(
            get_cylindrical_sincos_pos_embed(self.num_patches, self.hidden_dim),
            requires_grad=False
        )
